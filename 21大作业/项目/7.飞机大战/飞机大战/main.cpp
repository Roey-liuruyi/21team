...
public:
	...
	void draw() // 显示火箭相关信息	
	{
		...
		_stprintf(s, _T("已经存活了%d秒"), liveSecond);
		settextcolor(RGB(243,211,231)); // 设定文字颜色
		settextstyle(40, 0, _T("楷体"));//  设定文字大小、样式
		outtextxy(WIDTH * 0.50, 20, s); //  输出文字内容
...
void startup()  //  初始化函数
{
	...
	loadimage(&im_bk, _T("background.jpg")); // 导入背景图片
