...
int main()
{
	...
	{
		putimage(0, 0, &img);
		setlinecolor(RGB(68, 206, 246)); // 设置针颜色为68,206,246色
		line(0, 450, lineLength, 450); // 左边发射区域的一根针
		...
		for (i = 0; i < lineNum; i++) // 对所有旋转针进行遍历
		{
			...
			xEnd = lineLength * cos(-Angles[i]) + 450; // 计算针的末端坐标
			yEnd = lineLength * sin(-Angles[i]) + 450;
			setlinecolor(RGB(38, 50, 56)); // 设定旋转针的颜色为蓝色
			if (i == lineNum - 1) // 最新发射的一根针，设定颜色为红色
				setlinecolor(RGB(242, 190, 69));
			line(450, 450, xEnd, yEnd); // 绘制一根针
		}
		...
		if (_kbhit() && rotateSpeed != 0) // 如果按键，并且旋转速度不等于0
		{
			...
			if (input == ' ') // 如果为空格键
			{
				...
				xEnd = lineLength * cos(-Angles[lineNum - 1]) + 450; //新增针的末端坐标
				yEnd = lineLength * sin(-Angles[lineNum - 1]) + 450;
				line(450, 450, xEnd, yEnd); // 绘制出这根新增加的针
		...
		setfillcolor(HSVtoRGB(0.1166, 0.7149 + (lineNum / 120.0), 0.9451)); // 绘制中间的圆盘，针越多，其颜色越鲜艳
		setlinecolor(HSVtoRGB(0.1166, 0.7149, 0.9451)); // 设置圆盘线条颜色为红色
		fillcircle(450, 450, 60); // 绘制中间的圆盘
		TCHAR s[20]; // 定义字符串数组
